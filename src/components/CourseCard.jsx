...
export default function CourseGuide({
  ...
  if (admin == true)
    ...
    return (
      <Link className="bg-light-green p-5 rounded-lg flex flex-col justify-center gap-3">
        <h1 className="text-xl font-bold">{course}</h1>
        <p className="text-zinc-500 text-sm font-light">Mentor: {mentor}</p>
        <progress max={100} value={progress} className={progress > 0 ? "block" : "hidden"}></progress>
      </Link>
    );
}
